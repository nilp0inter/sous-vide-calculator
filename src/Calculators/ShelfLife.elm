...
view isMetric model =
    div [ class "p-6 bg-white rounded-lg shadow" ]
        [ h2 [ class "text-2xl font-bold mb-4 text-gray-800" ] [ text "Shelf-Life & Storage Timer" ]
        , p [ class "text-gray-600 mb-4" ]
            [ text "Determine how long pasteurized, chilled food can be safely stored." ]
        , div [ class "bg-purple-50 p-4 rounded-md" ]
            [ h3 [ class "font-semibold mb-2" ] [ text "Planned Inputs:" ]
            , ul [ class "list-disc pl-5 space-y-1 text-sm text-gray-700" ]
                [ li [] [ text ("Current Refrigerator Temperature (" ++ (if isMetric then "°C" else "°F") ++ ")") ]
                ]
            ]
        ]
